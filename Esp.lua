...
function EspObject:Render()
	...
	if visible.box.Visible then
        ...
        local weaponColor
        if self.options == self.interface.teamSettings.enemy then
            if self.weapon == "Knife" then
                weaponColor = Color3.new(1, 0, 0) 
            elseif self.weapon == "Gun" then
                weaponColor = Color3.new(0, 0.25, 0.75) 
            else
                weaponColor = Color3.new(0, 1, 0)
            end
        else
		    box.Color = parseColor(self, options.boxColor[1]);
        end

		box.Color = weaponColor
